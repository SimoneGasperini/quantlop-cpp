...
lab1, lab2, lab3, lab4 = data.keys()
col1, col2, col3, col4 = "tab:blue", "tab:orange", "tab:green", "tab:purple"
alpha = 0.4
...
runtime2 = np.array([data[lab2][str(q)] for q in qubits])
runtime3 = np.array([data[lab3][str(q)] for q in qubits])
runtime4 = np.array([data[lab4][str(q)] for q in qubits])
...
for k in range(runtime2.shape[1]):
    ax.plot(qubits, runtime2[:, k], color=col2, alpha=alpha)
...
ax.plot(qubits, np.mean(runtime4, axis=1), marker="s", label=lab4, color=col4)
for k in range(runtime4.shape[1]):
    ax.plot(qubits, runtime4[:, k], color=col4, alpha=alpha)
...
memory2 = np.array([data[lab2][str(q)] for q in qubits])
memory3 = np.array([data[lab3][str(q)] for q in qubits])
memory4 = np.array([data[lab4][str(q)] for q in qubits])
...
ax.plot(qubits, np.mean(memory2, axis=1), marker="s", label=lab2, color=col2)
for k in range(memory2.shape[1]):
    ax.plot(qubits, memory2[:, k], color=col2, alpha=alpha)
...
ax.plot(qubits, np.mean(memory4, axis=1), marker="s", label=lab4, color=col4)
for k in range(memory4.shape[1]):
    ax.plot(qubits, memory4[:, k], color=col4, alpha=alpha)
ax.set_xlabel("Qubits", fontsize=18)
ax.set_xticks(qubits)
ax.set_xticklabels(qubits, fontsize=14)
